      ...
      <div className="mx-auto max-w-4xl rounded-[2rem] border border-red-200 bg-white/80 p-8 shadow-xl shadow-red-100 backdrop-blur dark:border-slate-700 dark:bg-slate-900/80 dark:shadow-slate-950/40">
          ...
          <div className="mt-8 space-y-3">
            {entries.map((entry) => {
              ...
              return (
                <div
                  key={entry.id}
                  className={`flex items-center justify-between rounded-[1.25rem] border px-5 py-4 ${
                    isMe
                ...
                >
                  ...
                  <span className="text-sm text-slate-600 dark:text-slate-300">{entry.total_xp} XP</span>
                </div>
              );
            })}
          </div>
        ...
        {!isInTop && myRank != null && (
          <div className="mt-6 border-t border-slate-200 pt-6 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-300">
            Ta position actuelle : <span className="font-semibold text-slate-900 dark:text-white">#{myRank}</span>
          </div>
        )}
      </div>
    </main>
