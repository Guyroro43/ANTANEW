...
export default async function Page() {
  ...
  const unlockedCount = (badges ?? []).filter((badge) => earnedMap.has(badge.id)).length;
          ...
          {(badges ?? []).map((badge, index) => {
            ...
            return (
              <Reveal key={badge.id} delay={index * 0.05}>
                <div
                  className={`rounded-[1.25rem] border p-5 transition dark:border-slate-700 ${
                    unlocked
                      ? 'border-yellow-300 bg-gradient-to-br from-white via-yellow-50 to-green-50 shadow-sm dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-900 dark:to-emerald-950'
                  ...
                  <p className="mt-3 flex items-center gap-1.5 text-xs font-semibold">
                    {unlocked ? (
                      <span className="flex items-center gap-1.5 text-green-700 dark:text-green-400">
                        ...
                        Débloqué le {formatDate(earnedAt as string)}
                      </span>
                    ) : (
